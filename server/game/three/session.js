...
class ThreeGameSession {
  ...
  initBoard() {
    this.board = new Map();
    // 明棋：visible；暗棋：hidden，owner 暂设为该阵营，翻开揭示真实身份。
    for (const { faction, row, col, type } of THREE_LAYOUT) {
      this.board.set(keyOf(faction, row, col), {
        piece: type,
        faction,
        hidden: this.isDark,
        crossedCenter: false,
  ...
  removePlayer(socketId) {
    for (const f of THREE_FACTIONS) {
      if (this.players[f] === socketId) {
        this.players[f] = null;
        // 移除该方全部棋子（淘汰），但避免重复移除
        if (!this.eliminated.has(f)) this.eliminate(f);
      }
    }
    // 若剩余非淘汰阵营 <=1，则结束
