...
function findTraditionalType(row, col) {
  return tradMap.get(`${row},${col}`) || null;
}

function getChaosValidMoves(board, row, col) {
...
function canDarkCapture(attacker, target) {
...
function getDarkValidMoves(board, row, col) {
  const cell = board[row]?.[col];
